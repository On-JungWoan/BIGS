...
import submodules.hold.code.src.utils.external.body_models as body_models


def construct_da_mano_pose(hand_mean):
    from submodules.hold.code.src.model.mano.specs import mano_specs as body_specs
...
class MANOServer(GenericServer):
    def __init__(self, betas, is_rhand):
        from submodules.hold.code.src.model.mano.specs import mano_specs
        from submodules.hold.code.src.utils.external.body_models import MANO

        mano_layer = MANO(
            model_path="misc/body_models/mano",
            is_rhand=is_rhand,
            batch_size=1,
