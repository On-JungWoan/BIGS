...
import submodules.hold.common.metrics as metrics


def compute_bounding_box_centers_torch(vertices):
    assert isinstance(vertices, torch.Tensor)
    assert len(vertices.shape) == 2
    
    min_v = vertices.min(axis=0)[0]
    max_v = vertices.max(axis=0)[0]
    
    bbox_centers = (min_v + max_v) / 2
    return bbox_centers
...
def eval_icp_first_frame(data_pred, data_gt, metric_dict):
    ...
    from submodules.hold.code.src.utils.icp import compute_icp_metrics
    ...
    if 1 < 0:
        ...
        # path = 'sfm_init/COLMAP.obj'
        path = 'sfm_init/HLoc.obj'
        path = 'sfm_init/mesh_cano_object_step_misc.obj'
        obj_mesh = trimesh.load_mesh(path)
        
        v3d_o_ra = Vector3dVector(np.array(obj_mesh.vertices))
        faces_o = Vector3iVector(np.array(obj_mesh.faces))
        source_mesh = TriangleMesh(v3d_o_ra, faces_o)
