...
from submodules.hold.code.src.model.mano.server import MANOServer
from submodules.hold.code.src.model.obj.server import ObjectServer
from submodules.hold.code.src.utils.eval_modules import compute_bounding_box_centers


def map_deform2eval_torch(verts, scale, _normalize_shift):
    assert isinstance(verts, torch.Tensor) and isinstance(_normalize_shift, torch.Tensor)
    device = verts.device
    
    conversion_matrix = torch.tensor([[1, 0, 0], [0, -1, 0], [0, 0, -1]], dtype=torch.float32, device=device)
    # normalize_shift = np.array(
    #     [0.0085238, -0.01372686, 0.42570806]
    # )  # dummy vec at all time for HO3D

    normalize_shift = _normalize_shift.clone()
    normalize_shift[0] = normalize_shift[0] * -1

    src_verts = verts.clone()

    src_verts = torch.matmul(src_verts, conversion_matrix)
    src_verts = src_verts * scale
    src_verts = src_verts + normalize_shift
    return src_verts.unsqueeze(0)
...
def load_data(sd_p):
    ...
    misc_ps = sorted(glob(op.join('/'.join(sd_p.split("/")[:3]), "misc", "*")))
    # misc_ps = sorted(glob(op.join("logs", sd_p.split("/")[1], "misc", "*")))
    logger.info(f'[MISC PATH] {misc_ps[-1]}')
    ...
    if seq_name.split('_')[-1] == 'ho3d':
        seq_name = sd_p.split('/')[1]
    ...
    # trimesh.Trimesh(servers['left'].verts_c[0].cpu(), faces=faces['left'].cpu()).export('test.obj')
    ...
    dataset = np.load(f"preprocess/outputs/{seq_name}/data.npy", allow_pickle=True).item()
