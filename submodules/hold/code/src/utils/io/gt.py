...
import torch
import trimesh
from submodules.hold.common.body_models import build_mano_aa
from submodules.hold.common.transforms import project2d_batch, rigid_tf_torch_batch
...
from submodules.hold.code.src.utils.const import SEGM_IDS
# from preprocess_mask import SEQ_NAME

def load_data(full_seq_name):
    from smplx import MANO
    seq_name = [v[1] for k,v in SEQ_NAME.items() if k == full_seq_name][0]

    device = "cuda:0"
    human_model = MANO(
        "misc/body_models/mano", is_rhand=True, flat_hand_mean=False, use_pca=False
    ).to(device)

    data = torch.load(f"preprocess/processed/{seq_name}.pt")
    ...
    with open(f"preprocess/outputs/{full_seq_name}/corres.txt", "r") as f:
    ...
    obj_mesh = trimesh.load(
        f"preprocess/models/{obj_name}/textured_simple.obj",
        process=False,
    )
