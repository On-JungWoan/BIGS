...
def train_cd(args, wandb_exp_name=None):
    ...
    # if not args.use_controlnet:
    #     args.concepts_list[0]['instance_cond_dir'] = None
    ...
    if args.use_controlnet:
        ...
        controlnet_op = ControlNetModel.from_pretrained(args.controlnet_type, torch_dtype=weight_dtype)
                    ...
                    if args.controlnet_mode == "v0":
                        ...
                        with torch.no_grad():
                            down_block_res_samples, mid_block_res_sample = controlnet_op(
                                ...
                                encoder_hidden_states=controlnet_prompt_embeds,
                                controlnet_cond=cond_imgs,
                                conditioning_scale=args.controlnet_weight, # 1.,
                                guess_mode=False,
                                return_dict=False,
                            )
                        model_pred = unet(
            ...
            if accelerator.is_main_process:
                ...
                if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                    with torch.no_grad():
                        ...
                        #     else:
                        canon_op_img = Image.open(Path(__file__).parent / f"canon_{args.custom_object_name}.png")
                        # else:
                        #     # cv2.imwrite(str(Path(__file__).parent / f"canon_real_random_512.png"), np.random.randn(512, 512, 3)*255)
                        #     img_name = "canon_real_random_512.png" # "canon_real_random_1024.png" "canon_real_random.png" "canon_random_noise.png"
                        #     canon_op_img = Image.open(Path(__file__).parent / img_name)
                        ...
                        if args.cd_only_on_controlnet:
                            ...
                            images = [
                                pipeline(args.validation_prompt, num_inference_steps=25, generator=generator, eta=1.0, image=canon_op_img, controlnet_conditioning_scale=args.controlnet_weight).images[
                                    0
                                ]
                                for _ in range(args.num_validation_images)
                            ]              
