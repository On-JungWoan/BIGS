...
def get_crop_img_w_jnts(img, bbox, projected_jnts, rescale: float=1.2, resize: int=512):
    ...
    if projected_jnts is not None:
        ...
        return cropped_img, projected_jnts, cropped_img, projected_jnts
...
def do_ti(dataset, pipe, args):
    ...
    if args.gen_mask:
        scene_cameras = scene.getTrainCameras() if args.origin_mode else None
        people_infos = people_infos if args.origin_mode else [0]
        ...
        for pi in people_infos:
            if args.origin_mode:
                ...
                person_save_dir = overall_save_dir
            ...
            db_name = args.ti_save_dir.split('/')[-1]
            if not args.origin_mode:
                from glob import glob
                custom_ipt_imgs = sorted(glob(f'{args.custom_img_dir}/*'))
                custom_mask_imgs = sorted(glob(f'{args.custom_mask_dir}/*.jpg'))

            # Load all visible Cameras
            if args.origin_mode:
                pbar = tqdm(scene_cameras, desc="Processing scene masks (with mask)")
            else:
                pbar = tqdm(range(len(custom_ipt_imgs)))
                
            # processor
            # from diffusion_inversion.hed import HEDdetector
            # preprocessor = HEDdetector()
            from diffusion_inversion.pidi import PidiNetDetector
            ...
            for cam in pbar:
                ...
                is_valid = (cam_fid in valid_fids and cam_fid in human_mask_fname_dict) if args.origin_mode else True
                if is_valid:
                    if args.origin_mode:
                        gt_img_fname = cam.img_fname
                        gt_img_mask_fname = human_mask_fname_dict[cam_fid]
                    else:
                        gt_img_fname = custom_ipt_imgs[cam_fid]
                        gt_img_mask_fname = custom_mask_imgs[cam_fid]
                    ...
                    # load projected_joints
                    _data_idx = pi.fids.index(cam_fid) if args.origin_mode else 0
                    pj_jnts = pi.misc['projected_op_jnts'][_data_idx] if args.origin_mode else None
                    view_prompts = pi.misc['body_prompts'][_data_idx] if args.origin_mode else None

                    # Check front occlusion exists
                    if args.origin_mode and (not scene.occ_cam_dict is None):
                        if cam_fid in scene.occ_cam_dict:
                            occ_mask = scene.occ_cam_dict[cam_fid]
                            occ_mask = occ_mask.numpy()
                            occ_mask = cv2.resize(occ_mask, (cam.image_width, cam.image_height))
                    ...
                    if raw_projected_jnts is not None:
                        ...
                        joint_imgs = preprocessor(cv2.cvtColor(raw_masked_img, cv2.COLOR_RGBA2RGB))
    ...
    if args.optimize_cd:
        ...
        for pi in people_infos:
            ...
            cd_train_opt.instance_prompt = f"photo of a <new1> {args.custom_object_name}"  
            cd_train_opt.instance_prompt_wo_token = f"photo of a {args.custom_object_name}"  
            ...
            cd_train_opt.validation_prompt = f"photo of a <new1> {args.custom_object_name}"
            ...
            if args.cd_use_fullbody_prior:
                ...
                # cd_train_opt.train_batch_size = int(cd_train_opt.train_batch_size/2) if cd_train_opt.train_batch_size > 1 else 1
                cd_train_opt.prior_batch_size = args.cd_prior_batch_size
            cd_train_opt.origin_mode = args.origin_mode
            cd_train_opt.custom_object_name = args.custom_object_name
            cd_train_opt.controlnet_type = args.controlnet_type
            cd_train_opt.resume_from_checkpoint = args.resume_from_checkpoint
            ...
            if args.origin_mode:
                ...
                wandb_exp_name = args.ti_exp_name
...
if __name__ == "__main__":
    ...
    parser.add_argument("--custom_object_name", type=str, default="box")
    parser.add_argument("--custom_img_dir", type=str, default="/home/user/datasets/arctic_hold/images/arctic_s03_box_grab_01_1")
    parser.add_argument("--custom_mask_dir", type=str, default="preprocess/outputs/arctic_s03_box_grab_01_1/masks/object")
    parser.add_argument("--controlnet_type", type=str, default="lllyasviel/control_v11p_sd15_softedge") # "lllyasviel/sd-controlnet-canny" # "lllyasviel/control_v11p_sd15_openpose"
    ...
    os.makedirs(args.ti_save_dir, exist_ok=True)
