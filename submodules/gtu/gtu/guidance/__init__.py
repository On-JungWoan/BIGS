...
class DiffusionGuidance:
    def __init__(
        ...
        # input text
        self.prompt = f"a photo of a {obj_name}"
        self.negative_prompt = "distorted, low quality" 

        self.aux_pos_prompt = ", high quality, 8k uhd, realistic"
        self.aux_neg_prompt = ", unclear, cut-off, distorted, copy, mistake, low-grade, low resolution, lowest quality, lowres, bad anatomy, bad, hands, missing fingers, worst quality, nsfw"
        ...
        if textual_inversion_path is not None:
            self.prompt = f"a photo of a <new1> {obj_name}"
    ...
    def load_textual_inversion_path_dict(self, pids: List):
        ...
        text_inv_dict[pids[0]] = Path(self.textual_inversion_path) / Path(self.textual_inversion_expname)
    ...
    def get_loss(
        ...
        """
        loss_dict = dict()
        SAVE_EVERY_N_IMG = 300
        save_intermediate = True if iteration % SAVE_EVERY_N_IMG == 0 else False
        # save_intermediate = False # to prevent OOM
        ...
        # log loss images
        # if iteration % SAVE_EVERY_N_IMG == 0:
        if save_intermediate:
            # store input images
            ...
            del self.guidance_controlnet.last_loss_imgs
            if hasattr(self.guidance_controlnet, "last_inpaint_masks"):
                del self.guidance_controlnet.last_inpaint_masks
        
        # log recent loss
        # self.dg_loss_log_stack[pid].append(loss.detach().cpu().numpy())
        loss = self.dg_lambda[pid] * loss

        self.guidance_controlnet.last_loss_imgs = None
