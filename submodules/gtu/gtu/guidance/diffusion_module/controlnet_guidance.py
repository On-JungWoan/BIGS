...
def get_controlnet(name : str="inpaint_sd15", torch_dtype=torch.float32):
    ...
    elif name == "softedge_sd15":
        controlnet = ControlNetModel.from_pretrained("lllyasviel/control_v11p_sd15_softedge", torch_dtype=torch_dtype)
...
class OPControlNet(nn.Module):
    def __init__(
        ...
        if sd_version == "2.1":
            ...
            controlnets = [
                get_controlnet("softedge_sd15", self.dtype)
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            if self.do_cfg_rescale:
                ...
                control_image = self.get_control_image(cond_image, pred_rgb_512, guess_mode)
                down_block_res_samples, mid_block_res_sample = self.controlnet(
                    latents_noisy, # control_model_input,
                    t, # tt,
                    encoder_hidden_states=controlnet_prompt_embeds[1].unsqueeze(0),
                    controlnet_cond=[control_image[0][1]],
                    conditioning_scale=cond_scale,
                    guess_mode=guess_mode,
                    return_dict=False,
                )
                ...
                noise_pred = self.unet(
                    ...
                    t, # tt, 
                    encoder_hidden_states=self.embeddings[1].unsqueeze(0), # self.embeddings.repeat(batch_size, 1, 1),
                    cross_attention_kwargs=None,
                    # ControlNet part
                    down_block_additional_residuals=down_block_res_samples,
                    mid_block_additional_residual=mid_block_res_sample,
        ...
        if hold_last_img:
            ...
            if not (mask is None):
                _mask = mask.squeeze().detach().cpu().squeeze().numpy()
                _mask = _mask / _mask.max()
                _mask = (_mask * 255).astype(np.uint8)
                _mask = np.repeat(_mask[...,None], 3, axis=-1)
                self.last_inpaint_masks = _mask
            ...
            # import cv2; cv2.imwrite('test.png', self.last_loss_imgs[0].permute(1,2,0).cpu().detach().numpy()*255)
