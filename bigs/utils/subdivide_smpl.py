...
from bigs.models.modules.smpl_layer import SMPL
...
def _subdivide_smpl_model(smpl=None, smoothing=False, is_rhand=True, model_path="./misc/body_models/mano"):     
    if smpl is None:         
        smpl = MANO(model_path, create_transl=False, use_pca=False, flat_hand_mean=False, is_rhand=is_rhand)     
    ...
    new_smpl = MANO(model_path, create_transl=False, use_pca=False, flat_hand_mean=False, is_rhand=is_rhand)     
    new_smpl.lbs_weights = torch.from_numpy(attr["lbs_weights"]).float()
    posedirs = np.zeros((135, sub_vertices.shape[0] * 3)).astype(np.float32)
...
def subdivide_smpl_model(smpl=None, smoothing=False, n_iter=1, is_rhand=True):     
    if smpl is None:         
        SMPL_PATH = './misc/body_models/mano'
        smpl = MANO(SMPL_PATH, create_transl=False, use_pca=False, flat_hand_mean=False, is_rhand=is_rhand)          
    
    smpl.v_id = np.arange(778)[..., None]     
    for _ in range(n_iter):         
        smpl = _subdivide_smpl_model(smpl, smoothing, is_rhand)     
    return smpl