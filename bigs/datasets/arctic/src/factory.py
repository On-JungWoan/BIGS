...
def fetch_dataset_devel(args, is_train, seq=None, device=None):
    ...
    # if seq is not None:
    #     split = args.run_on
    ds = DATASET(args=args, split=split, seq=seq, device=device)
    return ds
...
def fetch_dataloader(args, mode, seq=None, device=None):
    if mode == "train":
        # reset_all_seeds(args.seed)
        dataset = fetch_dataset_devel(args, is_train=True, seq=args.seq, device=device)
        ...
        if "submit_" in args.extraction_mode:
            ...
            dataset = fetch_dataset_devel(args, is_train=False, seq=args.seq)
