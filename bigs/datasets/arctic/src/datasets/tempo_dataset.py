...
class TempoDataset(ArcticDataset):
    def _load_data(self, args, split):
        # data_p = f"{args.dataset_path}/data/arctic_data/data/feat/{args.img_feat_version}/{args.setup}_{split}.pt"
    ...
    def __init__(self, args, split, seq=None):
        ...
        # imgnames = dataset_utils.downsample(self.imgnames, split)
        # self.imgnames = imgnames
        logger.info(
            f"TempoDataset Loaded {self.split} split, num samples {len(self.imgnames)}"
        )
    ...
    def __getitem__(self, index):
        ...
        for imgname in imgnames:
            img_folder = f"{DATASET_ROOT}/data/arctic_data/data/images/"
            # inputs, targets, meta_info = self.getitem(
            targets = self.getitem(
                op.join(img_folder, '/'.join(imgname.split('/')[4:])), load_rgb=load_rgb
            )
            # if load_rgb:
