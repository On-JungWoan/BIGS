...
def eval_degree(pred, targets):
    ...
    pred_radian = pred["object.rad"].view(-1)  # radian
    gt_radian = targets['object']['radian'].view(-1)  # radian    
...
def eval_mpjpe_ra(pred, targets):
    joints3d_cam_r_gt = targets['add']['j3d.cam']
    joints3d_cam_l_gt = targets['anchor']['j3d.cam']
    joints3d_cam_r_pred = pred['mano_r.j3d.cam']
    joints3d_cam_l_pred = pred['mano_l.j3d.cam']
...
def eval_v2v_success(pred, targets, meta_info):
    ...
    v3d_cam_gt = unpad_vtensor(targets['object']['v.cam'], targets['object']['v_len'])
    v3d_cam_pred = unpad_vtensor(pred['object.v3d.cam'], targets['object']['v_len'])
    # v3d_cam_gt = unpad_vtensor(targets["object.v.cam"], targets["object.v_len"])
    # v3d_cam_pred = unpad_vtensor(pred["object.v.cam"], targets["object.v_len"])
...
def eval_contact_deviation(pred, targets):
